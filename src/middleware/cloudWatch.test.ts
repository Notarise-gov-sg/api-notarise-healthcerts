...
import { NotarisationResult, Observation } from "../types";
...
describe("test cloudwatch middleware for v1", () => {
  ...
  it("middleware should log unrecognised cert type from response", async () => {
    ...
    // let's assume the wrappedDocument has been notarized
    const newDoc = cloneDeep(wrappedDocumentV1);
    const observation: Observation = newDoc.data.fhirBundle.entry.find((ent) =>
